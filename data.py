...
# Always save next to this script, regardless of where you run it from
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
SAVE_DIR   = os.path.join(SCRIPT_DIR, 'f1_pipeline_outputs')
os.makedirs(SAVE_DIR, exist_ok=True)

# Save everything as a single dictionary — one file, zero extra dependencies
...
pipeline_data = {
    ...
    'y_train':       y_train,
    'y_test':        y_test,
    'X_train_smote': pd.DataFrame(X_train_res, columns=X_train.columns),
    'y_train_smote': pd.Series(y_train_res, name='is_winner'),
    'label_encoders': label_encoders,
}

save_path = os.path.join(SAVE_DIR, 'f1_pipeline.joblib')
joblib.dump(pipeline_data, save_path, compress=3)
